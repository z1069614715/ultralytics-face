...
from .metrics import bbox_iou, probiou, wasserstein_loss, bbox_inner_iou, bbox_mpdiou
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, mpdiou_hw=None):
        ...
        # iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True, ratio=1.4)
        # iou = bbox_mpdiou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, mpdiou_hw=mpdiou_hw[fg_mask])
        loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
        
        # nwd = wasserstein_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask], constant=24.4)
        # nwd_loss = ((1.0 - nwd) * weight).sum() / target_scores_sum
        # loss_iou = loss_iou * 0.7 +  nwd_loss * 0.3
...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        self.assigner = TaskAlignedAssigner(topk=3, num_classes=self.nc, alpha=0.5, beta=6.0)
    ...
    def __call__(self, preds, batch):
        ...
        if isinstance(self.bce, (SlideLoss,)):
            if fg_mask.sum():
