...
class FaceRandomCrop:
    def __init__(self, max_crop_ratio=0.2, p=0.5) -> None:
    ...
    def __call__(self, labels):
        ...
        n = len(cls)
        bboxes, keypoints = instances.bboxes, instances.keypoints
        for j in random.sample(list(range(len(cls))), k=round(self.p * n)):
            im, box, keypoint = self.random_crop_and_apply_keypoint(im, instances, j)
            bboxes[j] = box
            keypoints[j] = keypoint
        new_instances = Instances(bboxes, instances.segments, keypoints, bbox_format="xyxy", normalized=False)
        
        labels["img"] = im
        labels["instances"] = new_instances
        return labels

    def random_crop_and_apply_keypoint(self, im, instances, idx):
        box = instances.bboxes[idx]
        x1, y1, x2, y2 = list(map(int, box))
        h, w = y2 - y1, x2 - x1
        # 0:top 1:down 2:left 3:right
        random_mask_flag = np.random.randint(0, 4)
        if random_mask_flag in [0, 1]:
            crop_length = np.random.rand() * self.max_crop_ratio * h
        else:
            crop_length = np.random.rand() * self.max_crop_ratio * w
        
        # crop
        if random_mask_flag == 0:
            im[y1:y1+int(crop_length), x1:x2] = [114, 114, 114]
            y1 = y1 + int(crop_length)
        elif random_mask_flag == 1:
            im[y2-int(crop_length):y2, x1:x2] = [114, 114, 114]
            y2 = y2 - int(crop_length)
        elif random_mask_flag == 2:
            im[y1:y2, x1:x1+int(crop_length)] = [114, 114, 114]
            x1 = x1 + int(crop_length)
        else:
            im[y1:y2, x2-int(crop_length):x2] = [114, 114, 114]
            x2 = x2 - int(crop_length)

        # apply keypoints
        keypoints = instances.keypoints[idx]
        nkpt = keypoints.shape[0]
        if nkpt != 0:
            xy = keypoints[..., :2]
            visible = keypoints[..., 2]
            out_mask = (xy[:, 0] < x1) | (xy[:, 1] < y1) | (xy[:, 0] > x2) | (xy[:, 1] > y2)
            visible[out_mask] = 0
            keypoints = np.concatenate([xy, visible[..., None]], axis=-1).reshape(nkpt, 3)
        
        return im, np.array([x1, y1, x2, y2]), keypoints
